...
# Function to send NMEA sentences
def send_nmea_sentence(sentence):
    uart.write(sentence + b'\r\n')  # Send the NMEA sentence with a newline character
...
def parse_gpgsv(gpgsv_sentence):
    try:
        ...
        if len(parts) >= 7:
            num_sentences = int(parts[1]) if parts[1].isdigit() else 0
            sentence_num = int(parts[2]) if parts[2].isdigit() else 0
            num_sv_in_view = int(parts[3]) if parts[3].isdigit() else 0

            # Process SV (Satellite in View) data here
            sv_data = [int(sv) if sv.isdigit() else 0 for sv in parts[4:]]
        ...
        print("UnicodeError: Could not parse GPGSV sentence")
